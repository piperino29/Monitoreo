...
import { filtroJuanR } from "../utils/Limpieza";
...
const Vehicular = () => {
  ...
  if (loading)
    ...
    const Tecnico = filtroJuanR(data);
  Tecnico &&
    Tecnico.map((item) => {
      ...
      <Box component="main" sx={{ flexGrow: 1, py: 2 }}>
        <Container maxWidth="x1">
          ...
          <Grid item xs={12} sm={12} md={12}>
              <Tabla
                sx={{ height: "100%" }}
                title={<Typography variant="h8" sx={{ fontWeight: 'bold' }}>{"Juan Francisco Rodriguez > "}{Tecnico.length}
                </Typography>}
                ots={Tecnico}
